...
# flannel uses host-local, flannel.exe, and sdnbridge so copy that to the correct location
...
$na = Get-NetRoute | Where { $_.DestinationPrefix -eq '0.0.0.0/0' -and $_.NextHop -like '192.*' } | Select-Object -Property ifIndex
$managementIP = (Get-NetIPAddress -ifIndex $na[0].ifIndex -AddressFamily IPv4).IPAddress

#set info and save
$cniJson.delegate.AdditionalArgs[0].Value.Settings.Exceptions = $serviceSubnet, $podSubnet, '192.168.7.0/24', '192.168.6.0/24' #, ($managementIP + "/32")
$cniJson.delegate.AdditionalArgs[1].Value.Settings.DestinationPrefix = $serviceSubnet
$cniJson.delegate.AdditionalArgs[2].Value.Settings.DestinationPrefix = ($managementIP + "/32")
